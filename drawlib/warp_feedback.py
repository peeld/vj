"""
warp_feedback.py — reusable GPU feedback-loop pixel modifier.

The FeedbackLoop class wraps a ping-pong RGBA buffer pair and a
rotation/zoom/ripple feedback kernel. To use it:

    from warp_feedback import FeedbackLoop, FeedbackParams
...
The only constraint on "draw anything into curr" is that it writes into
loop.curr as a flat float32 array of length (H * W * 4), RGBA order, 0-1.

If you want to seed the loop with an external image:
    loop.write_rgba(some_float32_HxWx4_array)  # writes into curr
    loop.advance()                              # make it the prev frame

Screen-space smear vectors
--------------------------
A coarse vector field (default 16×16) drives a per-pixel UV offset that is
applied *after* zoom/rotate, so trails smear in the field's direction rather
...
Built-in patterns: "swirl", "rightward", "leftward", "upward", "downward",
...
class FeedbackParams:
    ...
    # shifted by (smear_field_vector * smear_strength).  The field is a coarse
    # grid bilinearly interpolated across the screen — set it via
    # FeedbackLoop.set_smear_pattern() or .set_smear_vectors().
...
def _feedback_kernel(
    ...
    # ── Screen-space smear ────────────────────────────────────────────────
    # Bilinearly sample the vector field at the current (post-zoom/rotate)
    # screen position and offset the sample point along that vector.
    # smear_strength is in UV units (e.g. 0.01 = 1 % of screen width).
...
def _make_smear_pattern(name: str, gw: int, gh: int) -> "np.ndarray":
    ...
    if name == "swirl":
        ...
        raise ValueError(
            ...
            "Valid: swirl, swirl_cw, rightward, leftward, upward, downward, "
            "inward, outward, diagonal, cross, turbulence"
        )
...
SMEAR_PATTERNS = [
    "swirl", "swirl_cw", "rightward", "leftward", "upward", "downward",
    "inward", "outward", "diagonal", "cross", "turbulence",
]
...
class FeedbackLoop:
    ...
    def set_smear_vectors(self, vectors: "np.ndarray") -> None:
        ...
        Strength is set via FeedbackParams.smear_strength.
        """
